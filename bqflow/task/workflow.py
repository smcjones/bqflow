...
def get_workflow(filepath:str=None, filecontent:str=None) -> Any:
  """Loads yaml for workflow, replaces newlines, and expands includes.
    ...
    Args:
     - filepath: (string) The local file path to the workflow yaml file to load.
...
def auth_workflow(config:Configuration, workflow:Any) -> None:
  """Adjust the "auth":"user|service" parameter based on provided credentials.
    ...
    Args:
      ...
      - workflow: (Recipe yaml) The yaml of a workflow.
    ...
    if __name__ == "__main__":
      ...
      execute(
        config = Configuration(
          client = '[CLIENT CREDENTIALS yaml STRING OR PATH]',
          user = '[USER CREDENTIALS yaml STRING OR PATH]',
          service = '[SERVICE CREDENTIALS yaml STRING OR PATH]',
